...
post '/bookmark' do
  ...
  if $db.exec_params('SELECT id FROM bookmarks WHERE user_id = $1 AND comic_id = $2', [current_user['id'], params[:comic_id]]).first.nil? # userがしおりをつけたことがないか，対象comicに対してしおりをつけたことがない場合
